...
class NLLSequenceLoss(nn.Module):
    ...
    def __init__(self):
        ...
        self.criterion = nn.NLLLoss(reduction='none',weight=torch.Tensor([0.3,0.7]).cuda())
...
#        print(x.size())
        ...
        f2, _ = self.gru1(f2)
        f2, _ = self.gru2(f2)
        #f2=torch.mean(f2,dim=1,keepdim=True)
        #f2=F.adaptive_avg_pool3d(f2,(f2.shape[0],f2.shape[2]))
