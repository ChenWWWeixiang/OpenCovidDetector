...
with open(args.output_file,'w') as f:
    ...
    for a_res in ress:
        ...
        plt.plot(fpr, tpr, label='healthy, AUC={:.2f}'.format(metric.auc(fpr, tpr)))
        fpr, tpr, thresholds = metric.roc_curve(y_one_hot[:,1], norm_x[:,1])
        plt.plot(fpr, tpr, label='CAP, AUC={:.2f}'.format(metric.auc(fpr, tpr)))
        fpr, tpr, thresholds = metric.roc_curve(y_one_hot[:,2], norm_x[:,2])
        plt.plot(fpr, tpr, label='ILD, AUC={:.2f}'.format(metric.auc(fpr, tpr)))
        fpr, tpr, thresholds = metric.roc_curve(y_one_hot[:,3], norm_x[:,3])
        plt.plot(fpr, tpr, label='COVID, AUC={:.2f}'.format(metric.auc(fpr, tpr)))
