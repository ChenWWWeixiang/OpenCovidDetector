...
res=np.load('../ipt_results/results/reader_study.npy')
pre=res[:,0]
gt=res[:,1]
...
READER_E=np.zeros(200)
for id in range(5,6):
    workbook=xlrd.open_workbook("reader_test "+str(id)+".xlsx")
    answer=open('answer.txt','r')
    answer=answer.readlines()
    a_n=np.array([an.split('\t')[0] for an in answer])
    gt=[1-int(an.split('/')[-1][0]=='c' or  an.split('/')[-3]=='LIDC' or an.split('/')[-3]=='reader_ex')for an in a_n]
    worksheet=workbook.sheet_by_index(0)
    name=worksheet.col_values(0,5)
    pre=worksheet.col_values(1,5)
